...
- 住所入力欄のフォーカス中に訪問履歴からのサジェストが出るか、選択で住所・電話番号が入るか
- 存在しない住所でルート計算した時、該当する訪問先カードがハイライトされ入力タブに戻るか
- 訪問先の削除・リセット・履歴削除の後、トーストの「元に戻す」で復元できるか
- 履歴のピン留めで「お気に入り」セクションに固定されるか（300件トリムでも消えないか）
- モバイル幅（lg未満）で結果画面の「ルート地図」トグルが機能し、状態が永続化されるか
- カスタム案でドラッグ&ドロップと上下ボタンの両方で並び替えできるか
