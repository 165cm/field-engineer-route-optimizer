...
### 10. 現場マークは `Visit` に持たせない

駐車場（P）・任意アルファベットのマークは `repair_visit_markers_v1` に訪問先IDをキーとして保存します（`VisitMarker`）。

`RoutePlan.order` は `Visit` のスナップショットなので、`Visit` に持たせると再計算前の古い値が表示されます。

「作業済み」と同様、表示時は必ず訪問先IDから引き直してください。削除された訪問先のマークは `visits` 変更時に破棄します。

駐車場マークは3状態（`ParkingState`）です。`undefined`＝未確認（グレー）、`'yes'`＝あり（◯・グリーン）、`'no'`＝なし（✗・レッド）で、タップするたびに 未確認 → あり → なし → 未確認 と巡回します。**「聞き忘れ（未確認）」と「聞いたが駐車場はなかった（なし）」を同じ見た目にしてはいけません。** boolean に戻すとこの区別が失われます。旧形式の `parking: true` は `normalizeParkingState` が `'yes'` として読み込みます。
...
- P・アルファベットのマークをリロード後も保持しているか
- Pボタンを連打したとき グレー → ◯（グリーン）→ ✗（レッド）→ グレー と巡回するか
