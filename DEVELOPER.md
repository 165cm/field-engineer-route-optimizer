...
### 9. カスタム順は「ピン留め」してから優先表示する

ユーザーが `↑↓` で並び替えると `customOrderPinned` が `true` になり、`repair_route_session_v1` に保存されます。

このフラグが立っている間は、再最適化（`handleOptimize`）で以下のように振る舞います。

- 生き残った訪問先の並び順を維持し、新規訪問先は末尾に追加する
- 結果画面の初期表示を自動案ではなく カスタム案（`id: 'X'`）にする

初回最適化直後の `customOrder` は自動案のシードでしかないため、フラグなしで「並び順あり」を判定してはいけません。

「自動順にリセット」ボタン（`resetCustomOrderToAuto`）でフラグを下ろすと、以降の再計算はおすすめ案を表示します。

### 10. 現場マークは「文字＝設定」「オンオフ＝訪問先」で分ける

現場マークは2つあり、責務を分けています。

- マークの**見た目**（文字・名前・有効/無効）は `Settings`（`otherMarkerLetter` / `otherMarkerLabel` / `otherMarkerEnabled`）。全訪問先で共通です。
- 訪問先ごとの**オン/オフ**は `repair_visit_markers_v1` に訪問先IDをキーとして保存（`VisitMarker`）。

1つめのマークは駐車場固定（`P`）で、設定画面でも変更できません。2つめの初期値は `T`（三脚）です。

`RoutePlan.order` は `Visit` のスナップショットなので、オンオフを `Visit` に持たせると再計算前の古い値が表示されます。「作業済み」と同様、表示時は必ず訪問先IDから引き直してください。削除された訪問先のマークは `visits` 変更時に破棄します。

マーク設定はルート計算に影響しないため、`buildRouteSettingsSignature` には含めません（変更しても「ルートが古い」警告は出しません）。

### 11. 最適化アルゴリズムはハイブリッド方式
...
**プラン上限との関係**
- 現時点の Pro 上限は 10 件（`src/lib/plan.ts`）。9 件以上ではヒューリスティック経路が動作する。
- N ≤ 8 のときは従来どおりブルートフォース経路が動作する。
- 上限を変更する際は `MAX_VISITS_PER_PARSE`（`server.ts`）も同期して更新すること。

### 12. プラン上限は `plan.ts` と `server.ts` を同期する
...
- カスタム案で並び替え → 訪問先を編集 → 再計算したとき、カスタム案が選択されたまま並び順が維持されるか
- 「自動順にリセット」で「維持中」表示が消え、次の再計算でおすすめ案が表示されるか
- P・2つめのマークのオンオフをリロード後も保持しているか
- 現場マーク設定で文字と名前を変更すると、全訪問先のボタン表示が一括で変わるか
